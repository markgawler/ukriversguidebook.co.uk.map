    ...
    public function execute()
    {
        ...
        if (JSession::checkToken() && $mapId != 0)
        {
            ...
            if ($auth) {
                ...
                // if we have modified the map and this is an autogenerated map change
                // the map type needs updating to be manual
                if ($deletesPts || $updatesPts || $newPts) {
                    $mapModel = new UkrgbmapModelMap;
                    $type = $mapModel->getMapType($mapId);
                    if ($type < 2) {
                        $mapModel->updateMapType(2, $mapId); // type 2 is Manual (0 & 1 auto generated)
                    }
                }
            } else {
